...
$result1 = date_sun_info(strtotime('+1 day', time()), $lat, $lon);
$sunr1 = date('H:i', $result1['sunrise']);
$suns1 = date('H:i', $result1['sunset']);
...
<div class="moonphasemoduleposition">
    <div class="moonrise1">
        ...
        <?php echo $lang['Moonrise']; ?>
        <div class="nextmoonrise">
            <strongnumbers>
                <?php echo date($timeFormatShort, (int) $MoonRise), "\n"; ?>
            </strongnumbers>
        </div>
    </div>
    <div class="mooncircle1">
        <div id='mooncircleinner'></div>
        <luminance1>
            <?php echo $lang['Luminance']; ?>
        </luminance1>
        ...
        <div class="moonphase">
            ...
            echo "";
            $day = date('l jS F Y');
            if ($day === date("l jS F Y", strtotime('2018-1-31'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2018-2-15'))) {
                echo 'Partial Solar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2018-7-13'))) {
                echo 'Partial Solar <oorange>Eclipse</oorange>>';
            } else if ($day === date("l jS F Y", strtotime('2018-7-27'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2018-7-28'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2018-8-11'))) {
                echo 'Partial Solar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-1-5'))) {
                echo 'Solar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-1-6'))) {
                echo 'Solar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-1-20'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-1-21'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-7-2'))) {
                echo 'Solar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-7-16'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-7-17'))) {
                echo 'Lunar <oorange>Eclipse</oorange>';
            } else if ($day === date("l jS F Y", strtotime('2019-12-26'))) {
                echo 'Solar <oorange>Eclipse</oorange>';
        ...
        <div class "moonposition"
            style="font-size:16px;line-height:20px;padding-top:2px;-webkit-transform:rotate(<?php echo $hemisphere; ?>deg);-moz-transform:rotate(<?php echo $hemisphere; ?>deg);-ms-transform:rotate(<?php echo $hemisphere; ?>deg);-o-transform:rotate(<?php echo $hemisphere; ?>deg);transform:rotate(<?php echo $hemisphere; ?>deg);">
    ...
    <div class="fullmoon1">
        <svg id="i-ban" viewBox="0 0 32 32" width="8" height="8" fill="#aaa" stroke="#aaa" stroke-linecap="round"
            stroke-linejoin="round" stroke-width="6.25%">
            <circle cx="16" cy="16" r="14" />
            <path d="M6 6 L26 26" />
        </svg>
        <?php echo $lang['Nextfullmoon']; ?> <br />
        <div class="nextfullmoon">
            <strongnumbers>
                ...
                if ($now1 < $moon1->full_moon()) {
                    echo date($dateFormat, (int) $moon1->full_moon());
                }
                //else {echo date($dateFormat, $moon1->next_full_moon() );}
                else
                    echo date($dateFormat, (int) $moon1->next_full_moon());
                ?>
            </strongnumbers>
        </div>
    ...
    <div class="newmoon1">
        <svg id="i-ban" viewBox="0 0 32 32" width="8" height="8" fill="#777" stroke="#777" stroke-linecap="round"
            stroke-linejoin="round" stroke-width="6.25%">
            <circle cx="16" cy="16" r="14" />
            <path d="M6 6 L26 26" />
        </svg>
        <?php echo $lang['Nextnewmoon']; ?>
        <div class="nextnewmoon">
            <strongnumbers>
                <?php // homeweatherstation create an instance of the next new moon
                $moon = new MoonPhase();
                $nextnewmoon = date($dateFormat, (int) $moon->next_new_moon());
    ...
    <div class="moonset1">
        ...
        <?php echo $lang['Moonset'] ?>
        <div class="nextmoonset">
            <strongnumbers>
                <?php echo date($timeFormatShort, (int) $MoonSet), "\n"; ?>
            </strongnumbers></span>

        </div>
    </div>
    <div class="meteorshower"><svg xmlns='http://www.w3.org/2000/svg' width='12px' height='12px' viewBox='0 0 16 16'>
